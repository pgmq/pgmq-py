# src/pgmq/async_operations.py
"""
Shared asynchronous PGMQ operations mixin.

All public async queue methods live here once. Backend clients inherit this mixin
...
TRANSACTIONAL_ASYNC_METHODS = frozenset(
    {
        "create_queue",
...
class AsyncPGMQueueOperationsMixin:
    """
    Shared asynchronous PGMQ API implemented once for all async backends.
    ...
    - ``_async_transaction_decorator`` class attribute (async transaction decorator)
    """

    _async_transaction_decorator = None

    def __init_subclass__(cls, **kwargs):
        super().__init_subclass__(**kwargs)
        decorator = getattr(cls, "_async_transaction_decorator", None)
        if decorator is None:
            return
        for name in TRANSACTIONAL_ASYNC_METHODS:
            if (
                name in AsyncPGMQueueOperationsMixin.__dict__
                and name not in cls.__dict__
            ):
                method = AsyncPGMQueueOperationsMixin.__dict__[name]
